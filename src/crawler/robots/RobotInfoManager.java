...
public class RobotInfoManager {
	...
	public RobotTxt getRobotTxt(String url) {
		...
		if(robotMap.containsKey(host)) return robotMap.get(host);
...
//		System.out.println(robotTxt);
	...
	public boolean isAllowed(String url) {
		...
		for(String link: allowedLinks) {
			...
			if(filePath.startsWith(link)) return true;
		}
		
		Set<String> disallowedLinks = robotTxt.getDisallowedLinks();
		for(String link: disallowedLinks) {
//			System.out.println("disallowed link: " + link.length() + ", " + link);
			if(link.charAt(link.length() - 1) == '/'
					&& link.substring(0, link.length() - 1).equals(filePath)) 
			{
				return false;
			}
			if(filePath.startsWith(link)) return false;
		}
		return true;
	}
	...
	public long getHostLastAccessTime(String url) {
		String host = getHostName(url);
		if(!crawlTime.containsKey(host)) return -1;
	...
	public boolean timeAllowed(String url) {
		...
		if(r == null || r.getCrawlDelay() == -1 || !crawlTime.containsKey(host)) return true;
