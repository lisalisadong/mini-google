...
public class DistributedCluster implements Runnable {
            ...
            for (int i = 0; i < localExecutors; i++)
                try {
                    ...
                    log.debug("Created a bolt executor " + key + "/" + newBolt.getExecutorId() + " of type "
                            + bolt.getLeft());
                    // System.out.println("Created a bolt executor " + key + "/"
                    // + newBolt.getExecutorId() + " of type " +
                    // bolt.getLeft());
                } catch (InstantiationException e) {
                    e.printStackTrace();
                } catch (IllegalAccessException e) {
    ...
    private void createRoutes(Topology topo, Config config) {
        ...
        for (String stream : topo.getBolts().keySet()) {
            ...
            // TODO:
            // Create a bolt for each remote worker, give it the same # of
            // entries
            // as we had locally so round-robin and partitioning will be
            // consistent
            int workerId = 0;
